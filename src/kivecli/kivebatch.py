...
class KiveBatch:
    ...
    def find_or_create(name: str, groups_allowed: Sequence[str]) -> "KiveBatch":
        ...
        with login() as kive:
            ...
            # Create new batch
            description = ""
            raw = kive.endpoints.batches.post(
                json=dict(
                    name=name, description=description, groups_allowed=groups_allowed
                )
            )
    ...
    def search(
        ...
        Args:
            ...
            description: Filter by batch description

        Yields:
            KiveBatch objects that match the search criteria
        """
        with login() as kive:
            query = build_search_query(name, description, page_size=page_size)
